...
router.post('BusinessProvider/register', function(req, res){
	...
	var errors = req.validationErrors();
	 BusinessProvider.findOne({ username: username}, function(err, user) {
        ...
        BusinessProvider.findOne({ email: email }, function(err, user) {
            if (err) { return next(err); }
            if (user) {
...
router.get('/BusinessProvider/login', function(req, res){
  ...
  passport.authenticate('local', {successRedirect:'/BusinessProvider/Profile', failureRedirect:'/users/login',failureFlash: true}),
  function(req, res) {
